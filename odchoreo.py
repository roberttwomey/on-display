...
# code = arm.set_servo_angle(angle=frontBackAngle, speed=50, mvacc=1000, wait=True, radius=-1.0)
code = arm.set_servo_angle(angle=stretchout, speed=50, mvacc=1000, wait=True, radius=-1.0)
...
# random.seed()
positions = [
    # [1.7,3.1,-1.7,37.2,1.3,-56.4,0.6],
    [0.0,2.5,0.0,37.3,0.0,-57.3,0.0],
    [84.1,-88.2,-1.3,84.1,6.0,11.7,-277.3],
    [297.7,-52.6,-9.0,120.5,12.9,22.4,242.0],
    [172.8,85.6,-255.4,131.1,-321.5,61.6,-102.9], # good far left
    [-161.5,73.4,-136.0,88.3,105.4,-20.4,-349.2], # upwards
    [-81.0,-72.5,59.9,214.2,200.3,83.4,271.0],
    [-164.5,14.6,-122.5,60.5,-39.3,-35.6,-221.9],
    [-352.0,-98.3,247.3,192.3,85.7,-23.2,-148.9],
    [-287.8,-4.5,26.0,34.5,-310.6,73.5,-282.9],
]

while True and not params['quit'] == True:
    try:
        try: 

            targetPose = []
            for limit in jointlimits:
                thisAngle = random.uniform(limit[0], limit[1])
                targetPose.append(thisAngle)

            targetPose = list(np.degrees(targetPose))
            print(targetPose)
            
            targetPose = positions[count]
            params['angle_speed']= random.uniform(4, 12)
            ...
            if arm.error_code == 0 and not params['quit']:
                ...
                code = arm.set_servo_angle(angle=targetPose, speed=params['angle_speed'], mvacc=params['angle_acc'], wait=True, radius=-1.0)
...
code = arm.set_servo_angle(angle=frontBackAngle, speed=20, mvacc=1000, wait=True, radius=-1.0)
